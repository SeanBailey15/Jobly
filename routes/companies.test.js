...
describe("GET /companies?filterParams", function () {
  test("works: filter by valid params - nameLike, minEmployees, and maxEmployees", async function () {
  ...
  test("bad request for missing value in query string", async function () {
    const resp = await request(app).get("/companies?nameLike=");
    expect(resp.statusCode).toEqual(400);
  });

  test("bad request for minEmployees greater than maxEmployees", async function () {
    const resp = await request(app).get(
